            ...
            <table class="table table-vcenter table-hover card-table text-nowrap">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                ...
                                <div class="avatar avatar-sm bg-primary-lt text-primary font-weight-bold"
                                     style="border-radius: 6px; width: 36px; height: 36px;">
                            ...
                            <div>
                                ...
                                @if($user->id === auth()->id())
                                    <span class="badge bg-green-lt text-green rounded-pill"
                                          style="font-size: 9px; padding: 1px 6px;">Anda</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <!-- Role Badge (Admin Purple, Kurir Yellow, Kasir Blue) -->
                        <td>
                            @php
                                $roleBadgeColor = match($user->role->name) {
                                  'admin' => 'purple',
                                  'kurir' => 'yellow',
                                  default => 'blue',
                                };
                            @endphp
                            <span class="badge bg-{{ $roleBadgeColor }}-lt rounded-pill px-2.5 py-1 font-weight-medium">
              <i class="ti ti-user-shield me-1 fs-3"></i> {{ $user->role->label }}
                        ...
                        <td>
                            @if($user->is_active)
                                <span
                                    class="badge bg-success-lt text-success rounded-pill px-2.5 py-1 font-weight-medium">
              <i class="ti ti-circle-check me-0.5 fs-3"></i> Aktif
            </span>
                            @else
                                <span
                                    class="badge bg-secondary-lt text-secondary rounded-pill px-2.5 py-1 font-weight-medium">
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-2">
                                <a href="{{ route('users.edit', $user) }}"
                                   class="btn btn-sm btn-icon btn-outline-primary" title="Ubah User"
                                   style="border-radius: 6px;">
                                ...
                                @if($user->id !== auth()->id())
                                    <form action="{{ route('users.destroy', $user) }}" method="POST"
                                        ...
                                        <button type="button"
                                                ...
                                                title="Hapus User" style="border-radius: 6px;"
                                                data-nama="{{ $user->name }}">
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            <div class="empty">
                                ...
                                <p class="empty-subtitle text-secondary mb-0">Klik tombol "Tambah User" di pojok kanan
                                    atas untuk mendaftarkan akun kasir atau pengelola baru.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
