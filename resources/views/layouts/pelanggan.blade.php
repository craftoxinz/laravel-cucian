        ...
        <div class="container-fluid">
                ...
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown" aria-label="Open user menu">
                        <div class="avatar avatar-sm fw-bold text-white bg-primary">
                            {{ strtoupper(substr(auth('pelanggan')->user()->nama, 0, 2)) }}
                        </div>
                    </a>
                    <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                        <div class="dropdown-header">
                            <div class="fw-semibold">{{ auth('pelanggan')->user()->nama }}</div>
                            <small class="text-muted">{{ auth('pelanggan')->user()->email }}</small>
                        </div>
            ...
            <div class="collapse navbar-collapse" id="sidebar-menu">
                <ul class="navbar-nav pt-lg-3">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pelanggan.orders.*') ? 'active' : '' }}" href="{{ route('pelanggan.orders.index') }}">
                            <span class="nav-link-icon"><i class="ti ti-receipt fs-2"></i></span>
                            <span class="nav-link-title">Pesanan Saya</span>
                        </a>
                    </li>
                ...
                <div class="mt-auto pb-3 d-none d-lg-block">
                    ...
                    <div class="nav-item dropdown px-3">
                        <a href="#" class="nav-link d-flex lh-1 text-reset p-0 align-items-center gap-2" data-bs-toggle="dropdown">
                            ...
                            <div>
                                <div class="text-white fw-medium small text-truncate" style="max-width: 130px;">{{ auth('pelanggan')->user()->nama }}</div>
                                <div class="mt-1 small text-white-50" style="font-size: 11px;">{{ auth('pelanggan')->user()->is_member ? 'Member' : 'Pelanggan' }}</div>
                            </div>
                        </a>
                        <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                            <div class="dropdown-header">
                                <div class="fw-semibold text-heading">{{ auth('pelanggan')->user()->nama }}</div>
                                <small class="text-muted">{{ auth('pelanggan')->user()->email }}</small>
                            </div>
                            <div class="dropdown-divider"></div>
                            <form method="POST" action="{{ route('pelanggan.logout') }}">
